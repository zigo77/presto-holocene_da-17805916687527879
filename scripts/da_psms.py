...
def psm_main(model_data,proxy_data,options):
    ...
    # proxy_estimate is assigned only inside the per-proxy loop, then returned
    # unconditionally at function exit. A zero-proxy input therefore raised
    # UnboundLocalError, masking the real cause (proxies failed to survive
    # da_load_proxies' filterTs gate). Exit early with a clear message instead.
    if n_proxies == 0:
        sys.exit(
            'ERROR: No proxies available for PSM matching (proxy_data["values_binned"] has shape '
            + str(proxy_data['values_binned'].shape) + '). '
            'This usually means lipd.filterTs in da_load_proxies dropped every entry — '
            'check the "No matches found for that expression" warnings above and verify the '
            'input pickle carries paleoData_inCompilation/paleoData_units metadata.'
        )
